#!/usr/bin/env python3
"""
Cross-Dataset Duplicate Detection Script

Checks for similar questions between a target dataset and a reference dataset
using the existing Question Deduplication pipeline (exact, fuzzy, and semantic matching).

Usage:
    python scripts/data_processing/check_cross_duplicates.py \
        --target outputs/final/final_paddy_relevant_only.csv \
        --reference "Data/RUS - Q and LLM - A.xlsx" \
        --output outputs/final/final_paddy_relevant_only_with_rus_check.csv
"""
...
class CrossDatasetDuplicateChecker:
    """
    Check for duplicate questions across two datasets.
    """
    ...
    def load_dataset(self, filepath: str, question_column: str = 'Question') -> pd.DataFrame:
        """
    ...
    def find_similar_question(self, 
                             ...
                             reference_indices: list) -> Tuple[bool, Optional[str], float, str]:
        """
        ...
        Args:
            ...
            reference_indices: List of reference question indices
        ...
        if self.config['deduplication']['exact']['enabled']:
            for ref_q, ref_idx in zip(reference_questions, reference_indices):
    ...
    def check_duplicates(self,
        ...
        """
        Check for duplicates between target and reference datasets.
        
        Args:
            target_file: Path to target CSV file
            reference_file: Path to reference dataset (CSV or Excel)
        ...
        """
        # Load datasets
        target_df = self.load_dataset(target_file, question_column)
        reference_df = self.load_dataset(reference_file, question_column)
        
        logger.info(f"Target dataset: {len(target_df)} questions")
        logger.info(f"Reference dataset: {len(reference_df)} questions")
        
        # Prepare reference questions
        reference_questions = reference_df[question_column].tolist()
        reference_indices = reference_df.index.tolist()
        
        # Initialize new columns
        target_df['has_similar_in_RUS'] = False
        target_df['similar_RUS_question'] = ''
        ...
        for idx, row in tqdm(target_df.iterrows(), total=len(target_df), desc="Processing"):
            ...
            target_df.at[idx, 'has_similar_in_RUS'] = has_similar
            target_df.at[idx, 'similar_RUS_question'] = similar_q if similar_q else ''
    ...
    def _generate_report(self, df: pd.DataFrame, output_file: str, matches_found: int):
        ...
        report = f"""
╔══════════════════════════════════════════════════════════════╗
║        Cross-Dataset Duplicate Detection Report              ║
╚══════════════════════════════════════════════════════════════╝

📊 Summary Statistics
─────────────────────────────────────────────────────────────
Total questions in target dataset:     {total:,}
Questions with similar match in RUS:   {matches_found:,}
...
"""
        ...
        matches_df = df[df['has_similar_in_RUS'] == True]
        if len(matches_df) > 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check for similar questions between two datasets"
    )
    parser.add_argument(
        '--target', '-t',
        required=True,
    ...
    parser.add_argument(
        '--reference', '-r',
        required=True,
        help='Path to reference dataset (CSV or Excel)'
    )
    parser.add_argument(
        '--output', '-o',
        required=True,
    ...
    parser.add_argument(
        '--question-column', '-q',
        default='Question',
        help='Name of column containing questions (default: Question)'
    )
    parser.add_argument(
        '--config', '-c',
        default='config.yaml',
    ...
    # Run duplicate check
    checker = CrossDatasetDuplicateChecker(args.config)
