#!/usr/bin/env python3
"""
Check questions.txt against RUS dataset and create CSV with marker columns.

Usage:
    python scripts/data_processing/check_questions_txt_vs_rus.py \
        --target Data/questions.txt \
        --reference "Data/RUS - Q and LLM - A.xlsx" \
        --output Data/questions_with_rus_check.csv
"""
...
class QuestionsTxtChecker:
    """
    Check questions from a text file against a reference dataset.
    """
    ...
    def load_reference_dataset(self, filepath: str, question_column: str = 'Question') -> pd.DataFrame:
        """
        Load reference dataset from CSV or Excel file.
        ...
        """
        logger.info(f"Loading reference dataset from {filepath}...")
    ...
    def check_duplicates(self,
        ...
        """
        Check for duplicates between target text file and reference dataset.
        
        Args:
            target_file: Path to target text file (one question per line)
            reference_file: Path to reference dataset (CSV or Excel)
            output_file: Path to output CSV file
            question_column: Name of column containing questions in reference
        
        Returns:
            DataFrame with duplicate information
        """
        # Load datasets
        target_questions = self.load_questions_txt(target_file)
        reference_df = self.load_reference_dataset(reference_file, question_column)
        
        logger.info(f"Target dataset: {len(target_questions)} questions")
        ...
        for idx, question in enumerate(tqdm(target_questions, desc="Processing"), start=1):
            ...
            results.append({
                'line_number': idx,
                'question': question,
                'has_similar_in_RUS': has_similar,
                'similar_RUS_question': similar_q if similar_q else '',
        ...
        self._generate_report(df, output_file, matches_found)
    ...
    def _generate_report(self, df: pd.DataFrame, output_file: str, matches_found: int):
        ...
        report = f"""
╔══════════════════════════════════════════════════════════════╗
║        Questions.txt vs RUS Dataset Report                   ║
╚══════════════════════════════════════════════════════════════╝

📊 Summary Statistics
─────────────────────────────────────────────────────────────
Total questions in questions.txt:      {total:,}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check questions.txt against RUS dataset"
    )
    parser.add_argument(
        '--target', '-t',
        required=True,
        help='Path to target text file (one question per line)'
    )
    parser.add_argument(
        '--reference', '-r',
        required=True,
    ...
    parser.add_argument(
        '--question-column', '-q',
        default='Question',
        help='Name of column containing questions in reference (default: Question)'
    )
    parser.add_argument(
        '--config', '-c',
        default='config.yaml',
    ...
    # Run duplicate check
    checker = QuestionsTxtChecker(args.config)
